...
export default class HibotchatDb {
    knex: any
    ...
    async getUserInfo(userId) {
        const { result: user } = await this.users.getOrCreateUser('hibot', userId)
    ...
    async initialize() {
        ...
        return this.knex
          .createTableIfNotExists('hibot_conversations', function(table) {
      ...
      async appendUserMessage(botId, userId, conversationId, { type, text, raw, data }) {
        ...
        const convo = await this.knex('hibot_conversations')
        ...
        return Bluebird.join(
          this.knex('hibot_messages')
            .insert(message)
            .then(),
    
          this.knex('hibot_conversations')
      ...
      async appendBotMessage(botName, botAvatar, conversationId, { type, text, raw, data }) {
        ...
        await this.knex('hibot_messages')
          .insert(message)
          .then()
      ...
      async createConversation(botId, userId, { originatesFromUserMessage = false } = {}) {
        ...
        await this.knex('hibot_conversations')
          .insert({
            botId,
            userId,
        ...
        const conversation = await this.knex('hibot_conversations')
      ...
      async listConversations(userId: string, botId: string) {
        const conversations = (await this.knex('hibot_conversations')
          .select('id')
          .where({ userId, botId })
        ...
        if (this.knex.isLite) {
          const lastMessagesDate = this.knex('hibot_messages')
      ...
      getConversationMessages(conversationId, fromId?: string): PromiseLike<any> {
        let query = this.knex('hibot_messages').where({ conversationId: conversationId })
      ...
      async getOrCreateRecentConversation(botId: string, userId: string, { originatesFromUserMessage = false } = {}) {
        console.log('socket de hibot llama esta function entra', botId, userId)
