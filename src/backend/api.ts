...
import rabbitHibot from './action'
...
export default async (bp: typeof sdk, db: Database) => { 
  ...
  const router = bp.http.createRouterForBot('channel-hibot', { checkAuthentication: false, enableJsonBodyParser: true })
  ...
  router.post(
    ...
    router.get('/conversations/:userId/:conversationId', async (req, res) => {
      ...
      async function sendNewMessage(botId: string, userId: string, conversationId, payload) {
        ...
        const { result: user } = await bp.users.getOrCreateUser('hibot', userId)
      ...
      const convertToTxtFile = async conversation => {
        ...
        const { result: user } = await bp.users.getOrCreateUser('hibot', conversation.userId)
